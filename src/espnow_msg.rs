...
//!
//! Clean break from the MicroPython JSON protocol — every message is a
//! 4-byte ASCII tag followed by a small binary body. The tag identifies
//! the message kind; the body shape is per-tag and documented next to
//! each encode/decode helper.
//!
...
//!
//! Currently encoded:
//! - `b"voc "` — vocalize: u8 icon index (see [`BubbleIcon::to_wire`]).
//!
...
/// byte we do not recognize — the receiver gets a sensible default
...
pub fn encode_vst(x: i32, pose: PoseId, mirror: bool, vx: f32) -> [u8; 4 + VST_BODY_LEN] {
    ...
    // means very fast cats just clamp to the rails — fine.
