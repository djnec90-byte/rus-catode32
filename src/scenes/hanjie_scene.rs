...
// Display constraints from the Python generator.
...
/// columns). An empty run is represented as `[0]` (len=1), matching Python's
/// `_run_lengths` fallback.
#[derive(Clone, Copy)]
struct Clue {
...
impl HanjieScene {
    ...
    /// Generate a random solution and clues. Mirrors Python's rejection loop
    /// (≤3 row groups, ≤2 col groups, 5+ filled cells) with a checkerboard
