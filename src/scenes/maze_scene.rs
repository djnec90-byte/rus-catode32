...
// Wall bitmask constants (match Python).
const WALL_N: u8 = 1;
const WALL_S: u8 = 2;
const WALL_E: u8 = 4;
const WALL_W: u8 = 8;
...
impl MazeScene {
    ...
    fn setup_reward_areas(&mut self, count: u32, rng: &mut u32) {
        ...
        for _ in 0..count {
            for _ in 0..100 {
                // Python: randint(1, gw - 4), randint(1, gh - 4).
...
impl Scene for MazeScene {
    fn enter(&mut self, ctx: &mut GameContext) {
        // Python only resets the round counter each visit; completion count
        // and coin tally carry over (matched here intentionally).
    ...
    fn update(
        ...
        // Held-B rewind with acceleration. Python advances this before the
        // edge-trigger handler; we keep the same ordering.
