...
//!
//! Ported from `micropython/src/scenes/pipes.py`. The board is generated by
//! DFS from the inlet so every game is guaranteed solvable. Water rises in
//! the inlet channel during a setup window, then advances cell-by-cell along
//! whatever rotations the player has set. Fat pipes slow the flow, corners
//! split into two legs. Difficulty scales with session wins.
