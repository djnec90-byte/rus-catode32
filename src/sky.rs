...
impl SkyRenderer {
    ...
    pub fn update(&mut self, ctx: &GameContext, dt: f32) {
        // Python scales sky dt by time_speed at the top of the main loop. Mirror that here.
        ...
        // TODO: port get_star_offset (time-of-night + seasonal drift) — small slow
        // shift of the star field over time.
    }
    ...
    fn configure_clouds(&mut self, weather: Weather) {
        ...
        // TODO: Python picks a random count between min and max each enter. Using
        // the lower bound for now keeps it deterministic until we have a per-scene PRNG.
    ...
    fn star_offset(&self, ctx: &GameContext) -> i32 {
        // Matches Python: drift 20px per game-hour and 60px per game-year.
    ...
    fn draw_moon(
        ...
        let frame = MOON_PHASE_FRAMES
            ...
            .copied()
            .flatten();
        // TODO: on "New" phase Python still draws a fill-only sprite to occlude stars.
