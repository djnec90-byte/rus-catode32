...
/// frame arrives. Mirrors the legacy MicroPython
/// `EspNowHandler._FLASH_DURATION` (5.5 s).
const HEARD_FLASH_DURATION: f32 = 5.5;

/// Cap on consecutive Hearing-triggered hearings before we stop
/// interrupting whatever the cat is doing. Mirrors the Python "crowd
/// control": if the last 3 entries in `recent_behaviors` are all
/// Hearing, the next vocalize only shows the corner bubble.
const HEARING_CROWD_CAP: usize = 3;

/// How often we re-broadcast the local cat's state during a visit
/// even when nothing changed. Matches the Python `_VST_HEARTBEAT`.
...
/// other device expects. Mirrors Python's `MainScene.ENTRY_X = 64`.
...
/// Inviter→invitee environment-sync broadcast interval. Matches the
/// legacy Python `_VENV_INTERVAL`.
