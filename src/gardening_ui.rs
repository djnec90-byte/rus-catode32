//! Interactive cursor modes for the plant system. Port of `gardening_ui.py`.
//!
...
/// Sort surfaces y_snap asc, then by layer (foreground last). Mirrors Python
/// so the starting cursor lands on the foreground floor.
...
/// Filter applied when cycling plants. Mirrors the Python `filter_fn` arg.
#[derive(Clone, Copy)]
...
impl PlantSelectionMode {
    ...
    pub fn draw(&self, ctx: &GameContext, renderer: &mut Renderer, env: &Environment) {
        ...
        let (pot_w, pot_h) = if let Some(spr) = pot_sprite(plant.pot) {
            ...
            // Ground plant: use the plant sprite height as the icon offset.
            let h = plant
                .seed
